...
protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(R.layout.noteslist_item_with_search);
...
SimpleCursorAdapter adapter = new SimpleCursorAdapter(
    ...
    dataColumns,                      // Column names to display
    viewIDs                           // View IDs to display the columns
);
...
adapter.setViewBinder(new SimpleCursorAdapter.ViewBinder() {
    public boolean setViewValue(View view, Cursor cursor, int columnIndex) {
        ...
        if (view.getId() == R.id.timestamp) {
            ...
            // 格式化日期时间显示
            String formattedDate = android.text.format.DateUtils.formatDateTime(
            ...
            ((android.widget.TextView) view).setText(formattedDate);
            return true; // 表示已经处理了这个视图的值设置
        }
        return false; // 使用默认的处理方式
    }
});
    ...
    if (query == null || query.isEmpty()) {
        ...
        setListAdapter(new SimpleCursorAdapter(this, R.layout.noteslist_item, cursor, dataColumns, viewIDs));
    } else {
        // 根据查询条件过滤笔记
        Cursor cursor = managedQuery(
            getIntent().getData(),
