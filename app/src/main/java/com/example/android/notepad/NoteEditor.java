...
public class NoteEditor extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTitleText = (EditText) findViewById(R.id.title); // 初始化 mTitleText
...
protected void onPause() {
    ...
    if (mCursor != null) {
        ...
        // Get the current note text and title
        String text = mText.getText().toString();
        String title = mTitleText.getText().toString(); // 获取标题文本
        ...
        if (isFinishing() && (length == 0)) {
            ...
            // For new notes, use the title from the title field
            updateNote(text, title);
            mState = STATE_EDIT;
        }
    }
